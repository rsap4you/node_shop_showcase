...
const hin = require("../language/hin");
    ...
    extractHeaderLanguage: (req, res, callback) => {
        var headerlang =
            req.headers["accept-language"] != undefined &&
                ...
                : "en";
        req.lang = headerlang;
        req.language = headerlang == "en" ? en : hin;
        localizify.add("en", en).add("hin", hin).setLocale(headerlang);

        callback();
    },

    getmassages: (language, massage, callback) => {
        localizify.add("en", en).add("hin", hin).setLocale(language);
